...
from django.contrib.auth.forms import PasswordChangeForm # Şifre formu için gerekli
...
# Veri İşleme ve Excel Kütüphaneleri
import pandas as pd
import io
from openpyxl.styles import Font, Alignment, PatternFill

# Yerel Modeller ve Planlayıcı
from .models import Doktor, IzinTalebi, Nobet, HastaneAyarlari
from .nobet_planner import NobetPlanlayici

# ==========================================
# 1. YÖNLENDİRME VIEW'LARI
# ==========================================

@login_required
def doktor_paneli_redirect(request):
    """
    Kullanıcı giriş yaptığında rolüne göre (Yönetici/Doktor)
    ilgili sayfaya yönlendiren fonksiyon.
    """
...
def takvim_redirect(request):
    """
    Takvim sayfasına tıklandığında güncel ay ve yıla yönlendirir.
    """
    bugun = date.today()
...
def doktor_paneli(request, yil, ay):
    ...
    # İzin Talebi Gönderme İşlemi (POST)
    if request.method == 'POST':
        ...
        if tarih_str:
            ...
            # Aylık izin limiti kontrolü
            mevcut_izin_sayisi = IzinTalebi.objects.filter(
                doktor=doktor, 
                tarih__year=izin_tarihi.year, 
                tarih__month=izin_tarihi.month
            ).count()
    ...
    # Takvim Navigasyonu Hesaplamaları
    ...
    context = {
        ...
        'onceki_yil': onceki_ay_tarih.year,
        'onceki_ay': onceki_ay_tarih.month,
        'sonraki_yil': sonraki_ay_tarih.year,
        'sonraki_ay': sonraki_ay_tarih.month
    }
...
def takvim_gorunumu(request, yil, ay):
    ...
    context = {
        ...
        'nobetler': gunlere_gore_nobetler,
        'onceki_yil': onceki_ay_tarih.year,
        'onceki_ay': onceki_ay_tarih.month,
        'sonraki_yil': sonraki_ay_tarih.year,
        'sonraki_ay': sonraki_ay_tarih.month
...
def yonetim_paneli(request):
    if request.method == 'POST':
        try:
            ...
            # Otomatik Planlayıcıyı Çalıştır
    ...
    context = {
        'varsayilan_yil': gelecek_yil,
        'varsayilan_ay': gelecek_ay,
        # 'doktor_sayisi' context'i de burada hesaplanıp gönderilebilir (önceki konuşmamızdaki gibi)
        'doktor_sayisi': Doktor.objects.count() 
    }
...
def export_takvim_excel(request, yil, ay):
    ...
    ayarlar = HastaneAyarlari.get_solo()
    bolumler = {
        'KIRMIZI': ayarlar.kirmizi_alan_doktor_sayisi, 
        'SARI': ayarlar.sari_alan_doktor_sayisi, 
        'YESIL': ayarlar.yesil_alan_doktor_sayisi
    }
    ...
    # Zorunlu nöbetleri işaretle
    df_raw['Doktor Adı'] = df_raw.apply(
        lambda row: f"{row['Doktor Adı']} (Zorunlu!)" if row['Zorunlu'] else row['Doktor Adı'], axis=1
    )

    # Pivot tablosu için pozisyon belirleme
    df_raw['Pozisyon_Numarasi'] = df_raw.groupby(['Tarih', 'Bölüm_Key']).cumcount() + 1
    
    df_raw['Pozisyon'] = df_raw.apply(
        lambda row: f"{row['Bölüm']} {row['Pozisyon_Numarasi']}" 
        if bolumler[row['Bölüm_Key']] > 1 else row['Bölüm'], axis=1
    )
    
    # Pivot Tablosu Oluştur
    df_pivot = df_raw.pivot_table(
        index='Tarih', columns='Pozisyon', values='Doktor Adı', aggfunc='first'
    ).fillna('')
    ...
    # Tarih formatını ve gün isimlerini ayarla
    ...
    df_stats = df_raw.groupby('Doktor Adı').agg(
        Toplam_Nöbet=('Doktor Adı', 'size'), 
        Hafta_Sonu_Nöbeti=('Hafta Sonu', 'sum'), 
    ...
    # Excel Dosyasını Oluştur (Bellekte)
    output = io.BytesIO()
...
class TekSeferlikSifreForm(PasswordChangeForm):
    """
    Kullanıcının şifreyi iki kez girmesini engelleyen,
    sadece yeni şifreyi tek seferde alan özel form.
    """
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Şifre tekrar alanını siliyoruz
        if 'new_password2' in self.fields:
            del self.fields['new_password2']

    def clean(self):
        # Şifre eşleşme kontrolünü devre dışı bırakıyoruz
